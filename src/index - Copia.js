...
bot.onText(/\/listas/, async (msg) => {
  try {
    ...
    await bot.sendMessage(msg.chat.id, '❌ Ocorreu um erro ao listar os canais. Por favor, tente novamente.');
  }
});
...
bot.onText(/\/ajuda/, async (msg) => {
  try {
    ...
    await bot.sendMessage(chatId,
      ...
      '3️⃣ Limites:\n' +
      '• Máximo 3 canais por usuário\n' +
      '• Divulgação a cada minuto\n\n' +
      '4️⃣ Comandos:\n' +
      '/start - Iniciar bot\n' +
      '/registrar - Novo canal\n' +
      '/minhascanais - Ver seus canais\n' +
  ...
  try {
    ...
    for (const category of categories) {
      ...
      for (const channel of channels) {
        try {
          const sent = await bot.sendMessage(channel.channelId, message, {
            parse_mode: 'HTML',
            disable_web_page_preview: true
          });
          
          // Pin the message for 30 seconds
          await bot.pinChatMessage(channel.channelId, sent.message_id);
          setTimeout(() => {
            bot.unpinChatMessage(channel.channelId, sent.message_id);
          }, 30000);
        } catch (error) {
          console.error(`Error posting to channel ${channel.title}:`, error);
        }
      }
    }
  } catch (error) {
